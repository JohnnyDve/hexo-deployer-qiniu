...
module.exports = class Deployer {
    ...
    deploy(public_dir){
        ...
        let dirsToRefresh = this.options.dirsToRefresh;
        return new Promise((resolve, reject) => {
            Promise.all(
                //附件上传
                traversal(public_dir,src=>that.upload(src,pathlib.win32.relative(public_dir,src).replace(/\\/g,"/"))
                    .finally(()=>{
                        console.log(src);
                    })
                )
                ...
                //目录刷新
                cdnManager.refreshDirs(dirsToRefresh,(respErr,respBody, respInfo)=>{
